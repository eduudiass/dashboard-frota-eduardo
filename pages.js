...
const Pages = {
    ...
    renderReceitas: function() {
        ...
        if (!dadosGlobais.receitas || dadosGlobais.receitas.length === 0) {
            container.innerHTML = `
                <div class="page-header">
                    <h1 class="page-title">💰 Receitas Mensais</h1>
                    <p class="page-subtitle">Controle de pagamentos recebidos</p>
                </div>
                <div style="text-align: center; padding: 60px 20px; color: var(--text-secondary);">
                    <div style="font-size: 64px; margin-bottom: 20px;">📊</div>
                    <h3>Nenhuma receita registrada</h3>
                    <p>Atualize a planilha com dados de receitas mensais</p>
                </div>
            `;
            return;
        }
        
        const totalRecebido = dadosGlobais.receitas.reduce((sum, r) => sum + r.total_recebido, 0);
        const totalPrevisto = dadosGlobais.receitas.reduce((sum, r) => sum + r.aluguel_previsto, 0);
        const percentualRecebido = totalPrevisto > 0 ? ((totalRecebido / totalPrevisto) * 100).toFixed(1) : 0;
        ...
        const html = `
            <div class="page-header">
                <div>
                    <h1 class="page-title">💰 Receitas Mensais</h1>
                    <p class="page-subtitle">Acompanhamento de pagamentos</p>
                </div>
                <div class="header-stats">
                    <div class="stat-badge success">
                        <span class="stat-value">${formatarMoeda(totalRecebido)}</span>
                        <span class="stat-label">Recebido</span>
                    </div>
                    <div class="stat-badge">
                        <span class="stat-value">${percentualRecebido}%</span>
            ...
            <div class="table-container">
                <table class="data-table">
                    ...
                    <tbody>
                        ${dadosGlobais.receitas.map(r => {
                            const percent = r.aluguel_previsto > 0 ? ((r.total_recebido / r.aluguel_previsto) * 100).toFixed(0) : 0;
                            return `
                                <tr>
                                    <td><strong>${r.placa}</strong></td>
                                    <td>${r.modelo}</td>
                                    <td>${formatarMoeda(r.aluguel_previsto)}</td>
                                    ${meses.map(m => `<td>${r.receitas_mensais[m] > 0 ? formatarMoeda(r.receitas_mensais[m]) : '-'}</td>`).join('')}
                                    <td class="success"><strong>${formatarMoeda(r.total_recebido)}</strong></td>
                                    <td><span class="status-badge ${percent >= 80 ? 'success' : percent >= 50 ? 'warning' : 'danger'}">${percent}%</span></td>
                                </tr>
                            `;
                        }).join('')}
                    </tbody>
    ...
    renderGastos: function() {
        ...
        if (!dadosGlobais.gastos || dadosGlobais.gastos.length === 0) {
            container.innerHTML = `
                <div class="page-header">
                    <h1 class="page-title">💸 Gastos Fixos</h1>
                    <p class="page-subtitle">Despesas mensais da frota</p>
                </div>
                <div style="text-align: center; padding: 60px 20px; color: var(--text-secondary);">
                    <div style="font-size: 64px; margin-bottom: 20px;">📊</div>
                    <h3>Nenhum gasto registrado</h3>
                    <p>Atualize a planilha com dados de gastos fixos</p>
                </div>
            `;
            return;
        }
        
        const totalMensal = dadosGlobais.gastos.reduce((sum, g) => sum + g.total_mensal, 0);
        const totalSeguro = dadosGlobais.gastos.reduce((sum, g) => sum + (g.seguro_anual / 12 || 0), 0);
        const totalIPVA = dadosGlobais.gastos.reduce((sum, g) => sum + (g.ipva_anual / 12 || 0), 0);
        ...
        const html = `
            <div class="page-header">
                <div>
                    <h1 class="page-title">💸 Gastos Fixos</h1>
                    <p class="page-subtitle">Despesas mensais recorrentes</p>
                </div>
                <div class="header-stats">
                    <div class="stat-badge danger">
                        <span class="stat-value">${formatarMoeda(totalMensal)}</span>
            ...
            <div class="kpi-grid" style="margin-bottom: 30px;">
                <div class="kpi-card">
                    ...
                    <div class="kpi-content">
                        <div class="kpi-label">Seguro (Mensal)</div>
                        <div class="kpi-value">${formatarMoeda(totalSeguro)}</div>
                    </div>
                </div>
                <div class="kpi-card">
                    <div class="kpi-icon warning">📄</div>
                    <div class="kpi-content">
                        <div class="kpi-label">IPVA (Mensal)</div>
                        <div class="kpi-value">${formatarMoeda(totalIPVA)}</div>
                    </div>
                </div>
                <div class="kpi-card">
                    ...
                    <div class="kpi-content">
                        ...
                        <div class="kpi-value">${formatarMoeda(totalCombustivel)}</div>
                    </div>
                </div>
            </div>
                        ...
                        ${dadosGlobais.gastos
                            ...
                            .map(g => `
                                <tr>
                                    ...
                                    <td>${g.modelo}</td>
                                    <td>${formatarMoeda(g.seguro_anual / 12 || 0)}</td>
                                    <td>${formatarMoeda(g.ipva_anual / 12 || 0)}</td>
                                    <td>${formatarMoeda(g.licenciamento / 12 || 0)}</td>
                                    <td>${formatarMoeda(g.combustivel_mensal || 0)}</td>
                                    <td class="danger"><strong>${formatarMoeda(g.total_mensal)}</strong></td>
                                </tr>
                            `).join('')}
                    </tbody>
                </table>
